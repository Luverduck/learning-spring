...
public class RequestBodyJsonController {
    ...
    //   핸들러 메소드의 매개변수 타입이 문자열인 경우 JSON 형태의 문자열로 바인딩된다.
    ...
    // @RequestBody 사용 - 핸들러 메소드의 매개변수에 직접 바인딩
    // - 핸들러 메소드의 매개변수 타입이 JSON 변환 결과 타입인 경우 해당 타입으로 직접 바인딩된다.
    @ResponseBody
    ...
    //   핸들러 메소드가 Java 객체를 반환할 경우 해당 타입의 필드값을 JSON 형태의 문자열로 변환하여 HTTP 응답 바디에 포함시킨다.
    @ResponseBody
    @PostMapping("/request-body-json-v5")
    public HelloData requestBodyJsonV5(@RequestBody HelloData helloData) {
        log.info("username={}, age={}", helloData.getUsername(), helloData.getAge());
        // @ResponseBody가 적용된 핸들러 메소드에서 Java 객체를 반환하면 JSON 형태의 문자열로 HTTP 응답 바디에 포함된다.
        return helloData;
    }
}
